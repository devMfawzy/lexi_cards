...
- `flutter_quill` for rich text editing/rendering
...
No inline images in cards, no sync — it's local-only. Decks can also only be created/deleted, not renamed, and there's no combined "study everything due" across decks — review is still one deck at a time. These are the obvious next steps if this grows past a portfolio piece.
